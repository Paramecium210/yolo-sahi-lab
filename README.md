...
results = model.predict(source="E:/VisualProject/ultralytics-main/data/mycoco/test_images", show=True, save=True)
```
...
for img_name in os.listdir(image_folder):
    if img_name.lower().endswith(".jpg"):
        ...
        result.export_visuals(export_dir=output_folder, file_name=f"{os.path.splitext(img_name)[0]}_pred.jpg")
        result.export_json(export_dir=output_folder, file_name=f"{os.path.splitext(img_name)[0]}.json")
```
